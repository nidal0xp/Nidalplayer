/**
 * NidalPlayer v5.8.0 — Samsung Smart TV (Tizen 4+)
...
// ── CONSTANTS ──────────────────────────────────────────────────────────────
var COMPANION = 'http://192.168.11.100:26101';
...
// ── STATE ──────────────────────────────────────────────────────────────────
var S = {
  lang: 'en', view: 'home', engine: (window.webapis && window.webapis.avplay) ? 'avplay' : (localStorage.getItem('np_engine') || 'html5'),
...
function load() {
  try {
    ...
    S.lang = savedLang || 'en';
    var savedEngine = localStorage.getItem('np_engine');
    if (savedEngine === 'html5' || savedEngine === 'avplay') {
      S.engine = savedEngine;
...
// ── REDIRECT RESOLVER & SCREENSHOT ENGINE ────────────────────────────────────
var _resolvedUrlCache = {};
function _resolveStreamUrl(url, callback) {
  if (!url || typeof url !== 'string' || url.indexOf('http') !== 0) {
    callback(url);
    return;
  }
  var entry = _resolvedUrlCache[url];
  if (entry && (Date.now() - entry.ts < 3000)) {
    logTV('_resolveStreamUrl cache hit', { original: url, cached: entry.finalUrl });
    callback(entry.finalUrl);
    return;
  }
  logTV('_resolveStreamUrl checking redirect for', url);
  var controller = window.AbortController ? new AbortController() : null;
  var timer = setTimeout(function() {
    if (controller) controller.abort();
    logTV('_resolveStreamUrl timed out, using original URL');
    callback(url);
  }, 1200);
  ...
  fetch(url, {
    ...
    if (!res.ok || res.status >= 400) {
      logTV('_resolveStreamUrl HTTP error ' + res.status + ', using original URL');
      callback(url);
      return;
    }
    var finalUrl = res.url || url;
    // CRITICAL: Preserve container extension if stripped by edge load balancer!
    if (url.indexOf('.m3u8') >= 0 && finalUrl.indexOf('.m3u8') < 0 && finalUrl.indexOf('.ts') < 0) {
      var qIdx = finalUrl.indexOf('?');
      if (qIdx >= 0) finalUrl = finalUrl.slice(0, qIdx) + '.m3u8' + finalUrl.slice(qIdx);
      else finalUrl += '.m3u8';
    } else if (url.indexOf('.ts') >= 0 && finalUrl.indexOf('.ts') < 0 && finalUrl.indexOf('.m3u8') < 0) {
      var qIdx = finalUrl.indexOf('?');
      if (qIdx >= 0) finalUrl = finalUrl.slice(0, qIdx) + '.ts' + finalUrl.slice(qIdx);
      else finalUrl += '.ts';
    }
    _resolvedUrlCache[url] = { finalUrl: finalUrl, ts: Date.now() };
    logTV('_resolveStreamUrl resolved URL', { original: url, final: finalUrl, status: res.status });
    callback(finalUrl);
  }).catch(function(err) {
    clearTimeout(timer);
    logTV('_resolveStreamUrl fallback to original', err ? err.message : err);
    callback(url);
  });
}

function takeScreenshot() {
  logTV('takeScreenshot requested');
  if (window.html2canvas) {
    window.html2canvas(document.body, {
      ...
      var vid = document.getElementById('tvMasterVideo');
      if (vid && vid.style.display !== 'none' && !vid.paused && vid.readyState >= 2) {
        try {
          var r = vid.getBoundingClientRect();
          if (r.width > 0 && r.height > 0) {
            ctx.drawImage(vid, r.left, r.top, r.width, r.height);
            logTV('takeScreenshot captured video frame', { x: r.left, y: r.top, w: r.width, h: r.height, vW: vid.videoWidth, vH: vid.videoHeight });
          }
        } catch(e) {
          logTV('takeScreenshot drawImage note: ' + (e ? e.message : e));
        }
      }
      var avPlaying = (AV._state === 'PLAYING' || AV._state === 'READY');
      if (avPlaying && S.activeCh) {
        var box = document.getElementById('tvLivePreviewBox');
        var br = (S.isFs) ? { left: 0, top: 0, width: 1920, height: 1080 } : (box ? box.getBoundingClientRect() : { left: 1088, top: 100, width: 804, height: 440 });
        if (br && br.width > 0) {
          ctx.fillStyle = '#0b101c';
          ctx.fillRect(br.left, br.top, br.width, br.height);
          ctx.fillStyle = '#10b981';
          ctx.font = 'bold 22px sans-serif';
          ctx.fillText('● HARDWARE AVPLAY ACTIVE (' + (S.isFs ? 'FULLSCREEN' : 'LIVE PREVIEW') + ')', br.left + 24, br.top + 50);
          ctx.fillStyle = '#ffffff';
          ctx.font = '18px sans-serif';
          ctx.fillText((S.activeCh.name || 'Live Channel') + ' • v5.8.0', br.left + 24, br.top + 85);
        }
      }
      try {
        var dataUrl = canvas.toDataURL('image/png');
        fetch('http://192.168.11.126:8765/api/screenshot', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ image: dataUrl })
        }).then(function(){
          logTV('Screenshot uploaded successfully!');
          toast('Screenshot Captured 📸');
        }).catch(function(e){
          logTV('Screenshot upload failed: ' + (e ? e.message : e));
        });
      } catch(te) {
        logTV('takeScreenshot toDataURL note: ' + (te ? te.message : te));
      }
    }).catch(function(err){
      logTV('html2canvas error: ' + (err ? err.message : err));
    });
    logTV('html2canvas not loaded');
  }
}
...
var AV = {
  ...
  play: function(url, rect, isLive) {
    ...
    logTV('AV.play request', { url: url, isLive: isLive, isFs: rect === FS_RECT, hasAVPlay: self.supported(), hasAndroid: !!window.AndroidBridge });
    ...
    var targetUrl = url;
    _resolveStreamUrl(targetUrl, function(finalUrl) {
      // 2. Samsung Smart TV Tizen AVPlay - only for fullscreen
      if (self.supported() && S.engine !== 'html5' &&
          rect.x === FS_RECT.x && rect.y === FS_RECT.y &&
          rect.w === FS_RECT.w && rect.h === FS_RECT.h) {
        try {
          try {
            var curSt = webapis.avplay.getState();
            if (curSt === 'PLAYING' || curSt === 'PAUSED' || curSt === 'READY') webapis.avplay.stop();
            if (curSt !== 'NONE') webapis.avplay.close();
          } catch(e){}

          logTV('AVPlay opening resolved stream with rect', { finalUrl: finalUrl, rect: self._rect || rect });

          webapis.avplay.open(finalUrl);
          self._url = finalUrl;
          self._state = 'OPEN';
          var curRect = self._rect || rect;
          var avObj = document.getElementById('avplayObject');
          if (avObj) {
            avObj.className = (curRect === FS_RECT) ? 'tv-avplayer video-fullscreen' : 'tv-avplayer video-preview-dock';
            avObj.style.left = curRect.x + 'px';
            avObj.style.top = curRect.y + 'px';
            avObj.style.width = curRect.w + 'px';
            avObj.style.height = curRect.h + 'px';
            avObj.style.display = 'block';
          }
          webapis.avplay.setDisplayRect(curRect.x, curRect.y, curRect.w, curRect.h);
          ...
          webapis.avplay.setListener({
            ...
            onerror: function(e){
              ...
              delete _resolvedUrlCache[targetUrl];
              _spin(false);
              self._teardown();
              if (finalUrl.indexOf('.ts') < 0 && (finalUrl.indexOf('.m3u8') >= 0 || finalUrl.indexOf('.mp4') >= 0)) {
                _html5Play(finalUrl, isLive, (self._rect || rect) === FS_RECT);
              }
            }
          });
          ...
          webapis.avplay.prepareAsync(
            function(){
              if (self._state === 'NONE' || self._state === 'ENDED') return;
              var targetRect = self._rect || rect;
              logTV('AVPlay prepareAsync SUCCESS, setting display rect', targetRect);
              var avObj = document.getElementById('avplayObject');
              if (avObj) {
                avObj.className = (targetRect === FS_RECT) ? 'tv-avplayer video-fullscreen' : 'tv-avplayer video-preview-dock';
                avObj.style.left = targetRect.x + 'px';
                avObj.style.top = targetRect.y + 'px';
                avObj.style.width = targetRect.w + 'px';
                avObj.style.height = targetRect.h + 'px';
                avObj.style.display = 'block';
              }
              try {
                webapis.avplay.setDisplayRect(targetRect.x, targetRect.y, targetRect.w, targetRect.h);
                webapis.avplay.setDisplayMethod('PLAYER_DISPLAY_MODE_LETTER_BOX');
                webapis.avplay.play();
                self._state = 'PLAYING';
                _spin(false);
                logTV('AVPlay state is now PLAYING with rect', targetRect);
              } catch(er){
                logTV('AVPlay play() call failed, falling back to HTML5', er ? er.message : er);
                _html5Play(finalUrl, isLive, targetRect === FS_RECT);
              }
            },
            function(e){
              logTV('AVPlay prepareAsync FAILED (' + (e ? (e.message || e.code) : '') + '), retrying with original stream URL');
              delete _resolvedUrlCache[targetUrl];
              self._teardown();
              setTimeout(function(){
                try {
                  ...
                  webapis.avplay.prepareAsync(function(){
                    try {
                      ...
                      self._state = 'PLAYING';
                      _spin(false);
                      logTV('AVPlay retry SUCCESS with original URL');
                    } catch(err2){
                      logTV('AVPlay retry play exception', err2 ? err2.message : err2);
                    }
                  }, function(err3){
                    logTV('AVPlay retry failed', err3 ? (err3.message || err3.code) : err3);
                  });
                } catch(err){
                  logTV('AVPlay retry open exception', err ? err.message : err);
                }
              }, 120);
            }
          );
          ...
          _html5Play(finalUrl, isLive, rect === FS_RECT);
        }
        return;
      }

      // 3. Fallback: HTML5 Video
      logTV('Using HTML5 video player with resolved stream', finalUrl);
      _html5Play(finalUrl, isLive, rect === FS_RECT);
    });
  },
  ...
  cycleAudio: function() {
    try {
      ...
      if (!total || !total.length) { toast('Audio: Default'); return; }
      var audios = [];
      for (var i = 0; i < total.length; i++) {
        if (total[i].type === 'AUDIO') audios.push(total[i]);
      }
      if (audios.length <= 1) {
        toast('Audio: 1/1 Default');
        return;
      }
      this._audioIdx = ((this._audioIdx || 0) + 1) % audios.length;
      var trk = audios[this._audioIdx];
      webapis.avplay.selectTrack('AUDIO', trk.index);
      var lang = 'Track ' + (this._audioIdx + 1);
      if (trk.extra_info) {
        try {
          var info = typeof trk.extra_info === 'string' ? JSON.parse(trk.extra_info) : trk.extra_info;
          if (info.language) lang = info.language;
        } catch(e){}
      }
      toast('Audio: ' + lang.toUpperCase());
    } catch(e) { logTV('cycleAudio error', e); }
  },
  ...
  cycleSubtitle: function() {
    try {
      if (!this.supported()) return;
      var total = webapis.avplay.getTotalTrackInfo();
      if (!total || !total.length) { toast('Subtitles: None'); return; }
      var subs = [];
      for (var i = 0; i < total.length; i++) {
        if (total[i].type === 'TEXT') subs.push(total[i]);
      }
      if (!subs.length) {
        toast('Subtitles: None Available');
        return;
      }
      this._subIdx = ((this._subIdx !== undefined ? this._subIdx : -1) + 1) % (subs.length + 1);
      if (this._subIdx === subs.length) {
        webapis.avplay.setSilentSubtitle(true);
        toast('Subtitles: OFF');
      } else {
        var trk = subs[this._subIdx];
        webapis.avplay.setSilentSubtitle(false);
        webapis.avplay.selectTrack('TEXT', trk.index);
        var lang = 'Sub ' + (this._subIdx + 1);
        if (trk.extra_info) {
          try {
            var info = typeof trk.extra_info === 'string' ? JSON.parse(trk.extra_info) : trk.extra_info;
            if (info.language) lang = info.language;
        ...
        toast('Subtitles: ' + lang.toUpperCase());
      }
    } catch(e) { logTV('cycleSubtitle error', e); }
  },

  toggleAspect: function() {
    try {
      if (!this.supported()) return;
      this._aspectMode = (this._aspectMode === 'PLAYER_DISPLAY_MODE_FULL_SCREEN') 
        ? 'PLAYER_DISPLAY_MODE_LETTER_BOX' 
        : 'PLAYER_DISPLAY_MODE_FULL_SCREEN';
      webapis.avplay.setDisplayMethod(this._aspectMode);
      toast(this._aspectMode === 'PLAYER_DISPLAY_MODE_FULL_SCREEN' ? 'Aspect: Stretch (16:9)' : 'Aspect: Fit (Original)');
    } catch(e){}
  },
  ...
  _teardown: function() {
    ...
    var avObj = document.getElementById('avplayObject');
    if (avObj) {
      avObj.className = 'tv-avplayer video-hidden';
    ...
    if (vid) {
      ...
      vid.load();
      vid.className = 'tv-master-video video-hidden';
    }
    if (this.supported()) {
      try {
...
function _html5Play(url, isLive, isFs) {
  var vid = document.getElementById('tvMasterVideo');
  if (!vid) { logTV('tvMasterVideo element not available'); return; }
  logTV('HTML5 starting playback', { url: url, isFs: isFs });
  _hlsDestroy();
  vid.className = isFs ? 'tv-master-video video-fullscreen' : 'tv-master-video video-preview-dock';
  vid.crossOrigin = 'anonymous';
  vid.playsInline = true;

  vid.onplaying = function(){ _spin(false); };
  vid.onwaiting = function(){ _spin(true); };
  vid.onerror = function(e){ logTV('HTML5 video error', e); };

  if ((url.indexOf('.m3u8') >= 0 || isLive || url.indexOf('/live/play/') >= 0) && window.Hls && Hls.isSupported()) {
    logTV('Hls.js attaching to master video element with URL: ' + url);
    _spin(true);
    _hls = new Hls({
      enableWorker: false,
    ...
    _hls.on(Hls.Events.MANIFEST_PARSED, function(){
      vid.play().then(function(){
        ...
        logTV('HLS play error', err ? err.message : err);
      });
    });
    _hls.on(Hls.Events.ERROR, function(e, d){
      if (d.fatal){
        logTV('HLS fatal error: ' + d.type + ' ' + d.details);
        _hlsDestroy();
        vid.src = url;
    ...
    logTV('HTML5 direct play with URL: ' + url);
    vid.src = url;
    vid.play().catch(function(err){ logTV('HTML5 direct play error', err ? err.message : err); });
  }
...
function syncPlaylist(pl, cb) {
  ...
  Promise.all([
    ...
    // 2. Fetch Live streams first so Live TV is ready immediately without delay!
    return _json(api+'&action=get_live_streams', 45000).then(function(d){
      S.liveCats=['ALL','FAVORITES'];
      var seen={};
      var rawChs = Array.isArray(d) ? d : [];
      S.live = rawChs.slice(0, 10000).map(function(x,i){
      ...
      logTV('Live channels synced', { count: S.live.length });
    }).catch(function(e){ logTV('get_live_streams error', e ? e.message : e); });
  }).then(function() {
    // 3. Fetch Movies and Series sequentially to preserve TV memory
    return Promise.all([
      _json(api+'&action=get_vod_streams', 45000).then(function(d){
        S.movieCats=['🔥 POPULAR','ALL','FAVORITES'];
        var seen={};
        var rawMvs = Array.isArray(d) ? d : [];
        S.movies = rawMvs.slice(0, 5000).map(function(x){
          var cid=String(x.category_id||'');
      ...
      _json(api+'&action=get_series', 45000).then(function(d){
        S.seriesCats=['🔥 POPULAR','ALL','FAVORITES'];
        var seen={};
        var rawSrs = Array.isArray(d) ? d : [];
        S.series = rawSrs.slice(0, 5000).map(function(x){
          var cid=String(x.category_id||'');
    ...
    if (S.view==='live') {
      if (!S.activeCh || (AV._state !== 'PLAYING' && AV._state !== 'OPEN')) {
        _renderLive();
      }
    }
    if (S.view==='movies') _renderMovies();
...
function _renderLive() {
  ...
  S.liveCats.forEach(function(cat) {
    var btn = _catBtn(cat, S.selCat.live===cat, function(){
      ...
      setTimeout(function(){
        var firstCh = document.querySelector('#liveChannelsList .tv-channel-row.active') || document.querySelector('#liveChannelsList .tv-channel-row');
        if (firstCh) focus(firstCh);
      }, 50);
    });
...
function _renderLiveChs() {
  ...
  items.slice(0, 60).forEach(function(ch) {
    ...
    row.onfocus = function(){
      // Scrolling / focusing updates the info card without starting/stopping the stream
    ...
    row.onclick = function(){
      var now = Date.now();
      if (row._lastClick && (now - row._lastClick < 350)) return;
      row._lastClick = now;

      var masterVid = document.getElementById('tvMasterVideo');
      var isAlreadyPlaying = S.activeCh && S.activeCh._id === ch._id &&
        (
          (masterVid && !masterVid.paused && masterVid.readyState >= 2 && !masterVid.classList.contains('video-hidden')) ||
          (AV._state === 'PLAYING' || AV._state === 'READY' || AV._state === 'OPEN')
        );
      if (isAlreadyPlaying) {
        // Second OK on the playing channel -> Expand to Fullscreen without reload
        logTV('Second OK on channel ' + ch.name + ' -> Fullscreen continuation');
        _openFs(ch, 0);
      } else {
        // First OK on this channel -> Start Preview immediately in dock
        logTV('First OK on channel ' + ch.name + ' -> Start Preview');
        _previewCh(ch, true);
      }
    };
  ...
  if (items.length && !S.activeCh) {
    var first = items[0];
    if (D.prevTitle) D.prevTitle.textContent = first.name;
    if (D.prevGrp)   D.prevGrp.textContent   = first.grp;
    if (D.prevLogo)  { D.prevLogo.src = first.logo||'icon.png'; D.prevLogo.onerror=function(){ this.src='icon.png'; }; }
  }
}
...
function _previewCh(ch, immediate) {
  if (!ch) return;
  S.activeCh   = ch;
  S.activeItem = ch;
  ...
  _previewTimer = setTimeout(function() {
    ...
    logTV('_previewCh starting for ' + ch.name, { url: url, rect: rect, immediate: !!immediate });
    // Show toast for first OK press (preview start)
    if (!immediate) {
      toast('Playing in preview. Press OK again for Fullscreen.');
    }
...
function _openFs(item, startSec) {
  ...
  // Hide all screens so video layer is completely visible
  ...
  // Check if this item is ALREADY playing in Live TV preview:
  var isSameItem = (S.activeCh && item && (S.activeCh._id === item._id || S.activeCh.url === item.url)) ||
                   (S.activeItem && item && (S.activeItem._id === item._id || S.activeItem.url === item.url));

  var masterVid = document.getElementById('tvMasterVideo');

  // A. HTML5 Video Seamless Expansion (0 delay, no reload)
  if (masterVid && !masterVid.paused && masterVid.readyState >= 2 && isSameItem) {
    logTV('_openFs: SEAMLESS HTML5 video expansion to fullscreen');
    masterVid.className = 'tv-master-video video-fullscreen';
    _osdShow();
    setTimeout(function() { if (D.osdPlayPause) focus(D.osdPlayPause); }, 100);
    return;
  }

  // B. Samsung Tizen AVPlay Seamless Expansion (0 delay, no reload)
  if (AV.supported() && (AV._state === 'PLAYING' || AV._state === 'READY' || AV._state === 'OPEN') && isSameItem) {
    logTV('_openFs: SEAMLESS EXPANSION of active preview stream to fullscreen without reload');
    AV._rect = FS_RECT;
    var avObj = document.getElementById('avplayObject');
    if (avObj) {
      avObj.className = 'tv-avplayer video-fullscreen';
      avObj.style.left = '0px';
      avObj.style.top = '0px';
      avObj.style.width = '1920px';
      avObj.style.height = '1080px';
      avObj.style.display = 'block';
    }
    try {
      ...
      logTV('_openFs: setDisplayRect failed, falling back to AV.play', e ? e.message : e);
    }
  }

  if (masterVid) masterVid.className = 'tv-master-video video-fullscreen';
  var avObj = document.getElementById('avplayObject');
  if (avObj) {
    avObj.className = 'tv-avplayer video-fullscreen';
    avObj.style.left = '0px';
  ...
  _osdShow();
  setTimeout(function() {
    if (D.osdPlayPause) focus(D.osdPlayPause);
  }, 100);
}
...
function _closeFs() {
  ...
  // If returning to Live TV and HTML5 video is playing, shrink back to preview box without reload!
  var masterVid = document.getElementById('tvMasterVideo');
  if (S.view === 'live' && masterVid && !masterVid.paused && masterVid.readyState >= 2) {
    logTV('_closeFs: SEAMLESS HTML5 shrink back to preview');
    masterVid.className = 'tv-master-video video-preview-dock';
    var vLive = document.getElementById('viewLive');
    if (vLive) vLive.classList.remove('hidden');
    if (D.topBar) D.topBar.classList.remove('hidden');
    setTimeout(function(){
      var chEl = document.querySelector('#liveChannelsList .tv-channel-row.active') || document.querySelector('#liveChannelsList .tv-channel-row');
      if (chEl) focus(chEl);
    }, 80);
    return;
  }

  // If returning to Live TV and AVPlay is playing, seamlessly shrink back to preview dock
  if (S.view === 'live' && AV.supported() && (AV._state === 'PLAYING' || AV._state === 'READY' || AV._state === 'OPEN')) {
    logTV('_closeFs: SEAMLESS AVPLAY shrink back to preview');
    var vLive = document.getElementById('viewLive');
    if (vLive) vLive.classList.remove('hidden');
    if (D.topBar) D.topBar.classList.remove('hidden');
    var rect = getPreviewRect();
    ...
    var avObj = document.getElementById('avplayObject');
    if (avObj) {
      avObj.className = 'tv-avplayer video-preview-dock';
      avObj.style.left = rect.x + 'px';
      avObj.style.top = rect.y + 'px';
      avObj.style.width = rect.w + 'px';
      avObj.style.height = rect.h + 'px';
      avObj.style.display = 'block';
    }
    try {
      ...
      logTV('_closeFs setDisplayRect failed:', e ? e.message : e);
    }
    setTimeout(function(){
      var chEl = document.querySelector('#liveChannelsList .tv-channel-row.active') || document.querySelector('#liveChannelsList .tv-channel-row');
      if (chEl) focus(chEl);
  ...
  if (masterVid) {
    masterVid.pause();
    masterVid.className = 'tv-master-video video-hidden';
  }
  AV._teardown();
  _hlsDestroy();
  showView(S.view);
...
function _osdShow() {
  ...
  S.osdTimer = setTimeout(function(){ if (D.fsOsd) D.fsOsd.classList.add('hidden'); }, 8000);
}
...
document.addEventListener('keydown', function(e) {
  ...
  if (isInput) {
    ...
    // Back key: blur input and focus Save button so user is never stuck
    ...
    // Enter / OK (13 / 29443) or Tab (9): advance to next input or to Save button
    if (c === 13 || c === 29443 || c === 9) {
      e.preventDefault();
    ...
    // Down Arrow (40): move down to next row or to Save button
    if (c === 40) {
      e.preventDefault();
      if (inp.id === 'cfgPlName') {
        var n = document.getElementById('cfgServer');
        if (n) focus(n);
      } else if (inp.id === 'cfgUser') {
        var n = document.getElementById('cfgPass');
        if (n) focus(n);
      } else if (inp.id === 'cfgServer' || inp.id === 'cfgPass') {
        inp.blur();
        var saveBtn = document.getElementById('btnSavePlaylist');
    ...
    // Up Arrow (38): move up to previous row
    if (c === 38) {
      e.preventDefault();
      if (inp.id === 'cfgServer') {
        var p = document.getElementById('cfgPlName');
        if (p) focus(p);
      } else if (inp.id === 'cfgPass') {
        var p = document.getElementById('cfgUser');
        if (p) focus(p);
      } else if (inp.id === 'cfgPlName' || inp.id === 'cfgUser') {
        inp.blur();
        var closeBtn = document.getElementById('btnCloseAddPlaylist');
        if (closeBtn) focus(closeBtn);
      }
      return;
    }

    // Allow standard text typing and cursor movement
    return;
  }
  ...
  if (S.isFs) {
    var isOsdHidden = !D.fsOsd || D.fsOsd.classList.contains('hidden');

    // Return / Back key: seamlessly return from Fullscreen to Live TV preview immediately
    if (c===27 || c===8 || c===4 || c===10009 || k==='XF86Back') {
      e.preventDefault();
      _closeFs();
      return;
    }
    ...
    if (c===412 || k==='MediaRewind') { e.preventDefault(); AV.seekRel(-10); _osdShow(); return; }
    if (c===417 || k==='MediaFastForward') { e.preventDefault(); AV.seekRel(10); _osdShow(); return; }
    if (c===427 || k==='ChannelUp') {
      e.preventDefault();
    ...
    if (k==='MediaTrackNext') { e.preventDefault(); _nextEp(); _osdShow(); return; }
    if (k==='MediaTrackPrevious') { e.preventDefault(); _prevEp(); _osdShow(); return; }

    // If OSD is hidden, waking it up on any D-Pad key or OK button
    if (isOsdHidden) {
      if (c===13 || c===29443 || c===38 || c===40 || c===37 || c===39) {
        e.preventDefault();
        _osdShow();
        if (D.osdPlayPause) focus(D.osdPlayPause);
        return;
    ...
    // OSD is visible: navigate between OSD buttons
    _osdShow();

    if (c===13 || c===29443) { // OK / ENTER
      e.preventDefault();
      var cur = document.activeElement;
      if (cur && cur.classList && cur.classList.contains('osd-btn')) {
        cur.click();
      } else {
        if (AV._state==='PLAYING') AV.pause(); else AV.resume();
      }
    ...
    if (c===37) { // LEFT
      e.preventDefault();
      var btns = Array.from(document.querySelectorAll('#osdBtnGroup .osd-btn')).filter(function(b){ return b.style.display !== 'none'; });
    ...
    if (c===38) { // UP
      e.preventDefault();
      if (S.activeItem && S.activeItem._type === 'live') {
        _switchCh(-1);
      } else {
        AV.seekRel(30);
      }
  ...
  if (c===27 || (c===8 && !isInput) || c===4 || c===10009 || k==='XF86Back') {
    ...
    if (S.modal) {
      ...
      // Level 1: Fullscreen active - close fullscreen and return to list with preview active
      _closeFs();
    } else if (S.view === 'live' && S.activeCh &&
               (AV._state === 'PLAYING' || AV._state === 'PAUSED' ||
                AV._state === 'READY' || AV._state === 'OPEN')) {
      // Level 2: Preview player active - stop preview, free resources, leave focus in list
      stopAll();
      // Keep focus on the currently selected channel in the live view
      var chEl = document.querySelector('#liveChannelsList .tv-channel-row.active') ||
                 document.querySelector('#liveChannelsList .tv-channel-row');
      if (chEl) focus(chEl);
    } else if (S.view!=='home') {
      // Level 3: Preview idle - navigate back to Home dashboard
      showView('home');
    }
    return;
  }
...
function _pushRemote() {
  ...
  var payload = {
    ...
    activeItem: S.activeCh ? {
      id: S.activeCh._id,
      name: S.activeCh.name,
      url: S.activeCh.url || S.activeCh.hls || '',
      grp: S.activeCh.grp || '',
      type: S.activeCh._type || 'live'
    } : (S.activeItem ? {
      id: S.activeItem._id,
      name: S.activeItem.name,
      url: S.activeItem.url || S.activeItem.hls || '',
      grp: S.activeItem.grp || '',
      type: S.activeItem._type || 'live'
    } : null),
    playback: {
      status: (S.playbackDuration > 0 || S.isFs) ? (S.playbackPaused ? 'paused' : 'playing') : 'ready',
      title: S.playbackTitle || (S.activeCh ? S.activeCh.name : (S.activeItem ? S.activeItem.name : '')),
      group: S.activeCh ? S.activeCh.grp : (S.activeItem ? S.activeItem.grp : ''),
      url: S.activeCh ? (S.activeCh.url || S.activeCh.hls || '') : (S.activeItem ? (S.activeItem.url || S.activeItem.hls || '') : ''),
...
function _handleRemote(act) {
  ...
  if (type === 'setLanguage' || type === 'changeLanguage') {
    var rawLang = (act.language || act.lang || 'en').toLowerCase();
    var newLang = rawLang.indexOf('ar') >= 0 ? 'ar' :
                  (rawLang.indexOf('fr') >= 0 ? 'fr' :
                  (rawLang.indexOf('es') >= 0 ? 'es' :
                  (rawLang.indexOf('de') >= 0 ? 'de' : 'en')));
    logTV('Switching TV language from remote to: ' + newLang);
    applyLang(newLang);
  ...
  if (type === 'setEngine' && act.engine) {
    S.engine = act.engine;
    logTV('Player engine switched to: ' + S.engine);
    toast('Engine: ' + S.engine);
    if (S.activeItem) AV.play(S.activeItem.url || S.activeItem.hls, S.isFs ? FS_RECT : getPreviewRect(), S.activeItem._type === 'live');
    return;
  }
  if (type === 'eval') {
    try {
      var evalRes = eval(act.code);
      logTV('eval result:', evalRes);
    } catch(e) {
      logTV('eval error:', e.message);
    }
    return;
  }
  ...
  if (type === 'tvKey' || type === 'dpad' || type === 'key' || type === 'nav') {
    var kk = String(act.key || act.dir || '').toUpperCase();
    if (kk === 'UP')    _nav('UP');
...
function init() {
  ...
  D.masterVid   = document.getElementById('tvMasterVideo');
  D.prevVid     = D.masterVid;
  D.fsVid       = D.masterVid;
  ...
  D.osdBtnBack  = document.getElementById('osdBtnBack');
  D.osdBtnRw    = document.getElementById('osdBtnRw');
  ...
  D.osdBtnFf    = document.getElementById('osdBtnFf');
  D.osdBtnAudio = document.getElementById('osdBtnAudio');
  D.osdBtnSub   = document.getElementById('osdBtnSub');
  D.osdBtnAspect= document.getElementById('osdBtnAspect');
  ...
  // OSD buttons
  if (D.osdBtnBack)   D.osdBtnBack.onclick   = function(){ _closeFs(); };
  if (D.osdBtnRw)     D.osdBtnRw.onclick     = function(){ AV.seekRel(-10); _osdShow(); };
  if (D.osdPrev)      D.osdPrev.onclick      = function(){ _prevEp(); _osdShow(); };
  if (D.osdPlayPause) D.osdPlayPause.onclick = function(){ if(AV._state==='PLAYING') AV.pause(); else AV.resume(); _osdShow(); };
  if (D.osdNext)      D.osdNext.onclick      = function(){ _nextEp(); _osdShow(); };
  if (D.osdBtnFf)     D.osdBtnFf.onclick     = function(){ AV.seekRel(10); _osdShow(); };
  if (D.osdBtnAudio)  D.osdBtnAudio.onclick  = function(){ AV.cycleAudio(); _osdShow(); };
  if (D.osdBtnSub)    D.osdBtnSub.onclick    = function(){ AV.cycleSubtitle(); _osdShow(); };
  if (D.osdBtnAspect) D.osdBtnAspect.onclick = function(){ AV.toggleAspect(); _osdShow(); };
  if (D.osdFavBtn)    D.osdFavBtn.onclick    = function(){ if(S.activeItem) _toggleFav(S.activeItem); _osdShow(); };
  ...
  // Hardware AVPlay / HTML5 Engine toggle button
  var btnAv = document.getElementById('btnToggleAvplay');
  if (btnAv) {
    var updateBtnAv = function() {
      btnAv.textContent = S.engine === 'html5' ? 'HTML5 ENGINE' : 'ENABLED';
      btnAv.classList.toggle('active', S.engine !== 'html5');
    };
    updateBtnAv();
    btnAv.onclick = function() {
      S.engine = S.engine === 'html5' ? 'avplay' : 'html5';
      try { localStorage.setItem('np_engine', S.engine); } catch(e){}
      updateBtnAv();
      toast('Engine: ' + (S.engine === 'html5' ? 'HTML5 Video' : 'Hardware AVPlay'));
    };
  }
  ...
  // Fast mobile companion remote polling (1.5s for real-time responsiveness)
  setInterval(_pollRemote, 1500);
